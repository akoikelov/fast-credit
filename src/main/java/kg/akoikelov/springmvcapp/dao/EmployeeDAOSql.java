...
import kg.akoikelov.springmvcapp.mappers.*;
...
public class EmployeeDAOSql implements EmployeeDAO {
  ...
  public Employee findByUsernameForJwt(String userName) {
    String sql =
        "select e.id, e.username, e.password, e.enabled, a.authority from employees e inner join "
            + "authorities a on e.username = a.username where e.username = ?";

    try {
      return jdbcTemplate.queryForObject(sql, new EmployeeJwtMapper(), userName);
    } catch (EmptyResultDataAccessException e) {
